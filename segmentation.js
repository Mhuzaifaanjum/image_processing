...
function generateHistogram() {
    ...
    // Draw histogram
    histogramCanvas.width = 512;  // Increased width for better visibility
    histogramCanvas.height = 256; // Increased height for better visibility
    ...
    histogramCtx.moveTo(30, histogramCanvas.height - 30);
    histogramCtx.lineTo(histogramCanvas.width - 20, histogramCanvas.height - 30);
    histogramCtx.stroke();

    // Y axis
    histogramCtx.beginPath();
    histogramCtx.moveTo(30, histogramCanvas.height - 30);
    histogramCtx.lineTo(30, 20);
    histogramCtx.stroke();

    // Draw histogram bars
    histogramCtx.fillStyle = 'black';
    for (let i = 0; i < histogramData.length; i++) {
        let barHeight = (histogramData[i] / maxCount) * (histogramCanvas.height - 60);
        histogramCtx.fillRect(30 + i * 2, histogramCanvas.height - 30 - barHeight, 2, barHeight);
    }
    ...
    for (let i = 0; i < 256; i += 32) {
        histogramCtx.fillText(i, 30 + i * 2, histogramCanvas.height - 10);
    }

    // Label Y axis
    histogramCtx.textAlign = 'right';
    for (let i = 0; i <= maxCount; i += maxCount / 5) {
        histogramCtx.fillText(Math.round(i), 20, histogramCanvas.height - 30 - (i / maxCount) * (histogramCanvas.height - 60));
    }
}
